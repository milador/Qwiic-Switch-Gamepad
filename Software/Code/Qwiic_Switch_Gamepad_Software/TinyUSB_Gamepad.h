...
TinyGamepad::TinyGamepad(void)
{
  this->usb_hid.setPollInterval(1);
  this->usb_hid.setReportDescriptor(desc_hid_report, sizeof(desc_hid_report));
  this->usb_hid.setStringDescriptor("Qwiic Switch Gamepad");
}

void TinyGamepad::begin(void)
{
  this->usb_hid.begin();
...
void TinyGamepad::write(void *report)
{
  wakeup();
  while(!isReady()) delay(1);
...
void TinyGamepad::press(uint8_t b)
{
...
void TinyGamepad::release(uint8_t b)
{
  b &= 0x7; // Limit value between 0..7
  _report.buttons &= ~((uint8_t)1 << b);
}
...
void TinyGamepad::releaseAll(void)
{
  _report.buttons = 0;
}

void TinyGamepad::buttons(uint8_t b)
{
  _report.buttons = b;
}


void TinyGamepad::xAxis(uint8_t a)
{
  _report.xAxis = a;
}


void TinyGamepad::yAxis(uint8_t a)
{
  _report.yAxis = a;
}

void TinyGamepad::move(uint8_t x,uint8_t y)
{
  _report.xAxis = x;
  _report.yAxis = y;
}

bool TinyGamepad::isReady(void)
{
	if (usb_hid.ready()) 
	  return true;
	return false;
